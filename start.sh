...
arachni_ip=$(cat /etc/hosts | grep `hostname` | awk '{print $1}')
echo "arachni rest server ip [${arachni_ip}]"
exec ruby $env_root/arachni-ui-web/bin/arachni_rest_server --address ${arachni_ip} $ARACHNI_PARAMS